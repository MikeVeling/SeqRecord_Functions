...
def feature_warning():
    ...
    """
    warn(
        "you are trying to edit sequence inside a feature, I have "
...
def save_SeqRecord_obj(SeqRecord_obj, path):
    """
...
#                              Base Functions                                 #
###############################################################################
def find_sequence(SeqRecord_obj, search_seq, forward_only=False, reverse_only=False):
    """
    ...
    forward_locs = [
        ...
        for m in re.finditer(
            "(?=" + search_seq.upper() + ")", str(SeqRecord_obj.seq).upper()
        )
    ]

    rc_search_seq = str(
        Seq(search_seq, SeqRecord_obj.seq.alphabet).reverse_complement()
    ).upper()

    reverse_locs = [
        m.start()
        for m in re.finditer(
            "(?=" + rc_search_seq + ")", str(SeqRecord_obj.seq).upper()
        )
    ]
...
def remove_sequence(
    ...
    SeqRecord_obj_ouptut = (
        SeqRecord_obj_intermediate[0:start] + SeqRecord_obj_intermediate[end:]
    )
    ...
    if maintain_ambiguious_features:
        for feature_input in SeqRecord_obj_intermediate.features:
            ...
            if (feature_start <= start and feature_end >= end) and not (
                feature_start == start and feature_end == end
            ):
                ...
                feature_start < start and feature_end > start and feature_end <= end
            ):  # End spanning
                feature.location._end = ExactPosition(start)
                if not supress_warnings:
                    feature_warning()
                SeqRecord_obj_ouptut.features.append(feature)
            elif start <= feature_start and end >= feature_end:  # Fully spanning case
                pass
            elif (
                feature_start >= start and feature_start < end and feature_end > end
            ):  # start spanning
                if not supress_warnings:
                    feature_warning()
                feature.location._end = ExactPosition(feature_end - (end - start))
                ...
                SeqRecord_obj_ouptut.features.append(feature)
            elif feature_start >= end and feature_end >= end:  # After case
                pass
            elif feature_start < start and feature_end <= start:  # Fully Before
                pass
            else:
                print(
                    "wut happened? you are trying to edit a sequence in a"
...
def add_sequence(
    ...
    if isinstance(sequence, str):
        sequence_useful = Seq(
            sequence, alphabet=SeqRecord_obj_intermediate.seq.alphabet
        )
    else:
        sequence_useful = sequence
    if strand == 1:
        SeqRecord_obj_ouptut = (
            SeqRecord_obj_intermediate[0:start]
            + sequence_useful
    ...
    if maintain_ambiguious_features:
        for feature_input in SeqRecord_obj_intermediate.features:
            ...
            feature_start = int(feature.location.start)
            feature_end = int(feature.location.end)
            if feature_start < start and feature_end > start:
                if not supress_warnings:
                    feature_warning()
                feature.location._end = ExactPosition(
                    feature_end + len(sequence_useful)
                )
                SeqRecord_obj_ouptut.features.append(feature)
    if make_feature:
        SeqRecord_obj_ouptut = add_feature(
            SeqRecord_obj_ouptut,
            start,
...
#                            Feature Functions                                #
###############################################################################
def identify_feature(SeqRecord_obj, feature):
    """
    ...
    if isinstance(feature, str):
        ...
        if feature not in SeqRecord_obj.features:
            raise Exception("Your target feature is not in your SeqRecord " + "object")
        target_features = [feature]
    elif isinstance(feature, list):
        for feature_test in feature:
            if feature_test not in SeqRecord_obj.features:
                raise Exception(
                    "Your target feature is not in your " + "SeqRecord object"
                )
        target_features = feature
    else:
...
def remove_feature(SeqRecord_obj, feature):
    """
    ...
    if len(target_features) == 0:
        warn("no features have been removed as we could not identify your " + "feature")
...
def add_feature_from_sequence(
    ...
    """
    SeqRecord_obj_output = copy.deepcopy(SeqRecord_obj)
    locations = find_sequence(
        SeqRecord_obj_output,
        search_seq,
    ...
    if not supress_warnings:
        if len(locations) == 0:
            warn(
                "could not find the sequence of the feature you are trying "
                + "to add. No features were added"
            )
        elif len(locations) > 1:
            warn(
                "Found many copies of your sequence, adding features to all "
                + "of them"
            )
    for location in locations:
        ...
        if location[2]:
            ...
            strand = 1
        SeqRecord_obj_output = add_feature(
            SeqRecord_obj_output,
            start,
            end,
            feature_name=feature_name,
...
def delete_seq_by_features(
    ...
    if not supress_warnings:
        if len(target_features) == 0:
            warn(
                "could not find the feature you are trying "
    ...
    while len(target_features) > 0:
        ...
        SeqRecord_obj_output = remove_sequence(
            SeqRecord_obj_output,
            start,
            end,
            maintain_ambiguious_features=(maintain_ambiguious_features),
            supress_warnings=supress_warnings,
        )
        target_features = identify_feature(SeqRecord_obj_output, feature_input)
    return SeqRecord_obj_output
...
def replace_sequence_by_locus(
    ...
    """
    SeqRecord_obj_intermediate = copy.deepcopy(SeqRecord_obj)
    SeqRecord_obj_intermediate = remove_sequence(
    ...
    SeqRecord_obj_output = add_sequence(
        SeqRecord_obj_intermediate,
        start,
        sequence,
...
def replace_sequence_by_feature(
    SeqRecord_obj,
    target_feature,
    sequence,
    make_feature=False,
    ...
    if len(target_features) == 0:
        warn(
            "no features have been replaced as we could not identify your " + "feature"
        )
    elif len(target_features) > 1:
        warn("multiple features have the same name, I will replace all of " + "them")
...
def test():
    ...
    save_SeqRecord_obj(SeqRecord_obj_cut_and_added, "test_out_cut_and_added.gbk")
    SeqRecord_obj_mod_atgs = add_feature_from_sequence(
        SeqRecord_obj_cut_and_added, "ATG", feature_name="Start"
    )
    save_SeqRecord_obj(SeqRecord_obj_mod_atgs, "test_moded_ATGs.gbk")
    SeqRecord_obj_remove_atgs = delete_seq_by_features(SeqRecord_obj_mod_atgs, "Start")
    save_SeqRecord_obj(SeqRecord_obj_remove_atgs, "test_removed_ATGs.gbk")
    replaced_GFP = replace_sequence_by_feature(
        SeqRecord_obj_remove_atgs,
        "replace GFP",
        "CATCATCAT",
        make_feature=True,
