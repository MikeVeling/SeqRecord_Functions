...
## Feature Functions
These functions allow you to mainpulate features within a SeqRecord object. It is important to point out that features are an indepente object, but SeqRecord objects often contain a list of features annotating the sequence. 
```
...
These functions are just spicific implimentations of the functions listed above. These allow you to do some high level functions like replace one sequence with another by combining both addition and removal of sequences
...
#Test
After import, run the follow# SeqRecord-Functions
This repository contains my own functions built on top of the Biopython SeqRecord objects. These functions allow for easy manipulation of these objects within python. These objects can be easily imported and exported from the standard GenBank file format for annotated sequences. This format is widely used by synthetic biologists for gene design and manipulation and integrates nicely into standard visualization tools like SnapGene and Benchling.
...
ing code in a folder with the test_in.gbk file included in this repository to ensure that everything is processing correctly 
```
test()
```
This test function ensures that all various  functions work properly. it must be run in a folder with test_in.gbk file.
